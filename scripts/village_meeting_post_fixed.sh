...
# Create the post content using a here document
read -r -d '' POST_CONTENT << 'EOF'
...
<p><strong>Sugihara proposal - Community currency with expiration dates:</strong><br>
Introduce a system like eumo to activate economic circulation within the village.</p>

<p><strong>UC proposal - Bartering system:</strong><br>
...
<p>Register for basic income like OpenAI Worldcoin, and use that income as village tax. This eliminates actual financial burden on villagers.</p>
...
<p>Grand plans were shared as the villages future vision:</p>
<ul>
...
ESCAPED_CONTENT=$(echo "$POST_CONTENT" | sed 's/\\/\\\\/g' | sed 's/"/\\"/g' | tr '\n' ' ' | sed 's/  */ /g')
