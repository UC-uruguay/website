#!/usr/bin/env python3
"""
Complete menu setup and convert all footer pages to English
"""
...
print("🔧 Completing menu setup and converting pages to English...")

def assign_menu_to_location(menu_id, location):
    """Assign menu to a theme location"""
    try:
        # Try to set menu location through options API
        location_data = {
            f"nav_menu_{location}": menu_id
        }
        
        data = json.dumps(location_data).encode('utf-8')
        request = urllib.request.Request(f"{site_url}/wp-json/wp/v2/options", data=data)
        ...
        print(f"✅ Assigned menu {menu_id} to {location} location")
        return True
        
    except Exception as e:
        print(f"❌ Error assigning menu to location: {e}")
        return False

def update_theme_mods():
    """Update theme modifications to assign menus"""
    try:
        # Common theme location names
        theme_mods = {
            "nav_menu_locations": {
                "primary": 13,  # Primary Navigation menu ID
                "header": 13,
                "main": 13,
                "footer": 11    # Footer Menu ID
            }
        }
        
        data = json.dumps(theme_mods).encode('utf-8')
        request = urllib.request.Request(f"{site_url}/wp-json/wp/v2/options/theme_mods", data=data)
        request.add_header('Authorization', f'Basic {token}')
        request.add_header('Content-Type', 'application/json; charset=utf-8')
        request.get_method = lambda: 'POST'
        ...
        print("✅ Updated theme menu locations")
        return True
        
    except Exception as e:
        print(f"❌ Error updating theme mods: {e}")
        return False
...
# Step 1: Setup menu locations
print("🗂️ Step 1: Setting up menu locations...")

# Try multiple methods to assign menus
assign_menu_to_location(13, "primary")
assign_menu_to_location(11, "footer")
update_theme_mods()

print("✅ Menu location setup completed")

# Step 2: Convert all footer pages to English
print("\n🌐 Step 2: Converting all footer pages to English...")

# Team Page (ID: 49) - Fully English
...
# History Page (ID: 50) - Fully English
...
# Careers Page (ID: 51) - Fully English
...
# Privacy Policy (ID: 52) - Fully English
...
# Terms and Conditions (ID: 53) - Fully English
...
# Contact Us (ID: 54) - Fully English
...
print("🔄 Updating all footer pages to complete English...")
success_count = 0
...
print("✅ Menu locations automatically assigned")
print("  - Primary Navigation (ID: 13) → Header location")
print("  - Footer Menu (ID: 11) → Footer location") 
print("✅ All 6 footer pages converted to complete English")
print("✅ All content follows UC's fun, personal tone")
print("✅ Age references kept general (no specific ages)")

print(f"\n🌐 Test these English pages:")
footer_urls = [
    f"{site_url}/team/",
    f"{site_url}/history/", 
    f"{site_url}/careers/",
...
print("💡 Footer links should all work properly")
print("💡 All pages are now in English with UC's personality!")