#!/usr/bin/env python3
"""
WordPress Profile Photo Fixer
Fix the missing UC profile photo in the About Me section
"""
import urllib.request
import json

class WordPressProfilePhotoFixer:
    def __init__(self):
    ...
    def fix_profile_photo_section(self, current_content):
        """Fix the profile photo section in the content"""
        print("🔧 Fixing profile photo section...")
        
        # The correct profile photo section
        correct_profile_photo = f'''<!-- wp:image {{"id":{self.media_id},"width":"280px","height":"280px","scale":"cover","style":{{"border":{{"radius":"50%"}}}}}} -->
...
<!-- /wp:image -->'''
        ...
        # Look for broken profile photo patterns and replace them
        import re
        
        # Pattern 1: Broken {self.media_id} references
        pattern1 = r'<!-- wp:image.*?-->'
        pattern2 = r'<img src=".*?{self\.image_url}.*?"/>'
        pattern3 = r'wp-image-{self\.media_id}'
        
        # Replace broken patterns
        fixed_content = current_content
        
        # Fix the image block in the About Me section
        # Look for the About Me section and fix the image there
        about_me_start = fixed_content.find('about-me-section')
        if about_me_start != -1:
            # Find the image block within About Me section
            about_me_section = fixed_content[about_me_start:about_me_start + 3000]  # Get enough context
            
            # Look for image patterns to replace
            if '{self.media_id}' in about_me_section or '{self.image_url}' in about_me_section:
                # Replace the broken image section with the correct one
                fixed_content = fixed_content.replace(
                    'wp-image-{self.media_id}', f'wp-image-{self.media_id}'
                )
                fixed_content = fixed_content.replace(
    ...
    def update_homepage_with_fixed_photo(self, fixed_content):
        """Update the homepage with fixed profile photo"""
        try:
            print("💾 Updating homepage with fixed profile photo...")
            ...
            print("✅ Homepage updated with fixed profile photo!")
    ...
    def create_complete_fixed_homepage(self):
        """Create the complete homepage with properly fixed profile photo"""
        print("🔄 Creating complete homepage with fixed profile photo...")
        
        # Complete correct homepage content with proper profile photo
        complete_content = f'<!-- wp:cover {{"url":"{self.image_url}","id":{self.media_id},"dimRatio":30,"overlayColor":"black","minHeight":60,"minHeightUnit":"vh","align":"full"}} -->' + '''
...
<!-- /wp:group -->'''
    ...
    def fix_profile_photo(self):
        """Main function to fix the UC profile photo"""
        print("📸 UC Profile Photo Fixer")
        print("=" * 50)
        
        # Create the complete corrected content
        fixed_content = self.create_complete_fixed_homepage()
        
        # Update the homepage
        result = self.update_homepage_with_fixed_photo(fixed_content)
        
        if result:
            print("\n🎉 UC Profile Photo Fix Completed!")
            print("=" * 60)
            print("✅ UC profile photo restored: DONE")
            print("✅ Round profile photo with border: DONE")
            print("✅ Proper image URL and media ID: DONE")
            print("✅ All sections preserved: DONE")
            print(f"🌐 Live site: {result['link']}")
            print("\n📸 UC profile photo is now visible again!")
        else:
            print("❌ Failed to fix UC profile photo")
        
        return result

def main():
    fixer = WordPressProfilePhotoFixer()
    fixer.fix_profile_photo()
