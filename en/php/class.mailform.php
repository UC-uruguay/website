...
class Mailform {
	...
	public function __construct() {
		...
		$this->from_address          = $rm_from_address;
	...
	// protected mail_address_check
	protected function mail_address_check( $name, $post, $bool ) {
	...
	// protected reply_address_set
	protected function reply_address_set( $name, $post ) {
	...
	// protected sanitize_post
	protected function sanitize_post( $p, $mode ) {
	...
	public function mail_send() {
		...
		if ( $this->from_address !== '' ) {
			$additional_headers = "From: ".$this->from_address;
			$additional_params  = '-f'.$this->from_address;
		}
		...
		if ( $this->reply_mail === 1 ) {
			...
			if ( $this->from_address !== '' ) {
				$thanks_additional_headers = "From: ".$this->send_name." <".$this->from_address.">";
				$thanks_additional_params  = '-f'.$this->from_address;
			}
